...
import { DashboardData, VaultMindTask } from "../types";
...
export class DashboardView extends ItemView {
	...
	private activePriorityFilter: "all" | "high" | "medium" | "low" | "none" =
		"all";
	...
	async onOpen() {
		...
		if (this.plugin.settings.showQuickStatsInDashboard !== false) {
			const statsSection = content.createEl("div", {
				cls: "vaultmind-section",
			});
			const statsHeader = statsSection.createEl("h3");
			const statsIcon = statsHeader.createEl("span", {
				cls: "section-icon",
			});
			setIcon(statsIcon, "bar-chart-2");
			statsHeader.createEl("span", { text: " Quick stats" });
			statsContainer = statsSection.createEl("div", {
				cls: "vaultmind-stats",
			});
		}

		// Tasks section (always shown)
		const tasksSection = content.createEl("div", {
			cls: "vaultmind-section",
		});
		const tasksHeader = tasksSection.createEl("h3", {
			cls: "tasks-header-with-filters",
		});
		tasksHeader.empty(); // Clear any existing content
		tasksHeader.addClass("vaultmind-tasks-header");

		// Title part
		const tasksTitlePart = tasksHeader.createEl("div", {
			cls: "tasks-title vaultmind-tasks-title-part",
		});
		const tasksIcon = tasksTitlePart.createEl("span", {
			cls: "section-icon",
		});
		...
		// Filter buttons
		const filterContainer = tasksHeader.createEl("div", {
			cls: "task-filters-inline vaultmind-filter-container",
		});
		...
		priorities.forEach((priority) => {
			const btn = filterContainer.createEl("button", {
				text: priority.label,
				cls: `filter-btn-inline ${
					this.activePriorityFilter === priority.value ? "active" : ""
				}`,
			});
			btn.addEventListener("click", () => {
				// Toggle filter - if clicking the same filter, reset to 'all'
				if (
					this.activePriorityFilter === priority.value &&
					priority.value !== "all"
				) {
					this.activePriorityFilter = "all";
				} else {
					this.activePriorityFilter = priority.value as
						| "all"
						| "high"
						| "medium"
						| "low";
		...
		if (this.plugin.settings.showGoalsInDashboard !== false) {
			const goalsSection = content.createEl("div", {
				cls: "vaultmind-section",
			});
			const goalsHeader = goalsSection.createEl("h3");
			const goalsIcon = goalsHeader.createEl("span", {
				cls: "section-icon",
			});
			setIcon(goalsIcon, "target");
			goalsHeader.createEl("span", { text: " Goals" });
			goalsContainer = goalsSection.createEl("div", {
				cls: "vaultmind-goals",
			});
		}
		...
		if (this.plugin.settings.showTimeTrackingInDashboard !== false) {
			const timeSection = content.createEl("div", {
				cls: "vaultmind-section",
			});
			const timeHeader = timeSection.createEl("h3");
			const timeIcon = timeHeader.createEl("span", {
				cls: "section-icon",
			});
			setIcon(timeIcon, "clock");
			timeHeader.createEl("span", { text: " Time tracking" });
			timeContainer = timeSection.createEl("div", {
				cls: "vaultmind-time",
			});
		}
	...
	private async getDashboardData(): Promise<DashboardData> {
		...
		for (const file of files) {
			...
			lines.forEach((line, index) => {
				...
				if (taskMatch) {
					...
					const parsedTask = parseTaskMetadata(
						text.replace(/^- \[([ x])\] /, "")
					);

					const taskObj: VaultMindTask = {
						id: `${file.path}-${index}`,
						content: text,
						file: file,
						filePath: file.path,
						line: index,
						completed,
						dueDate: parsedTask.dueDate || dueDate || undefined,
		...
		return {
			...
			time: {
				...
				currentSession:
					this.plugin.timeTracker.getActiveEntry() || undefined,
	...
	private renderQuickStats(container: HTMLElement, data: DashboardData) {
		...
		if (data.time.currentSession) {
			const sessionMinutes = Math.floor(
				(Date.now() -
					new Date(data.time.currentSession.startTime).getTime()) /
					(1000 * 60)
			);
		...
		const stats = [
			...
			{
				label: "Completed",
				value: data.quickStats.tasksCompletedToday,
				icon: "check-circle",
			},
			{
				label: "Overdue",
	...
	private renderTasks(container: HTMLElement, data: DashboardData) {
		...
		const createTaskElement = (
			...
			if (task.filePath && task.filePath.trim() !== "") {
				...
				console.warn(
					"VaultMind: Task without filePath:",
					task.content,
					"Task object:",
					task
				);
		...
		if (this.activeTagFilter) {
			const filterInfo = container.createEl("div", {
				cls: "active-tag-filter vaultmind-filter-info",
		...
		const pendingWithoutDatesPreview = this.sortTasksByPriority(
			this.applyPriorityFilter(
				this.applyTagFilter(
					data.tasks.pending?.filter((t) => !t.dueDate) || []
				)
			)
		);
		const hasFilteredTasks =
			...
			pendingWithoutDatesPreview.length > 0;

		if (!hasFilteredTasks && this.activePriorityFilter !== "all") {
			container.createEl("div", {
				cls: "no-filter-results vaultmind-no-results",
		...
		if (filteredOverdue.length > 0) {
			...
			const overdueIcon = overdueHeader.createEl("span", {
				cls: "task-group-icon",
			});
			setIcon(overdueIcon, "alert-triangle");
			overdueHeader.createEl("span", {
				text: ` Overdue (${filteredOverdue.length})`,
			});
			...
			if (filteredOverdue.length > maxOverdue) {
				const moreEl = overdueList.createEl("li", {
					text: `... and ${
						filteredOverdue.length - maxOverdue
					} more overdue tasks`,
					cls: "more-tasks-note",
				});
		...
		if (filteredToday.length > 0) {
			...
			const todayIcon = todayHeader.createEl("span", {
				cls: "task-group-icon",
			});
			setIcon(todayIcon, "calendar");
			todayHeader.createEl("span", {
				text: ` Today (${filteredToday.length})`,
			});
		...
		if (filteredUpcoming.length > 0) {
			...
			const upcomingIcon = upcomingHeader.createEl("span", {
				cls: "task-group-icon",
			});
		...
		const allPending = data.tasks.pending?.filter((t) => !t.dueDate) || [];
		...
		if (pendingWithoutDates.length > 0) {
			...
			const pendingIcon = pendingHeader.createEl("span", {
				cls: "task-group-icon",
			});
			...
			const hasFilter =
				this.activeTagFilter || this.activePriorityFilter !== "all";
			const headerText =
				hasFilter && allPending.length !== pendingWithoutDates.length
			...
			const renderPendingTasks = (count: number) => {
				pendingList.empty();
				pendingWithoutDates.slice(0, count).forEach((task) => {
				...
				if (pendingWithoutDates.length > initialShow) {
					const buttonContainer = pendingList.createEl("li", {
						cls: "show-more-container vaultmind-show-more-container",
					});

					const showMoreBtn = buttonContainer.createEl("button", {
						text: this.getShowMoreText(
							count,
							pendingWithoutDates.length
						),
						cls: "mod-cta vaultmind-show-more-btn",
					});
		...
		if (this.plugin.settings.showTagsInDashboard !== false) {
			// Default to true
			const projects = data.tasks.projects || [];
			if (projects.length > 0) {
				const projectsEl = container.createEl("div", {
					cls: "task-group",
				});
				const projectsHeader = projectsEl.createEl("h4");
				const projectsIcon = projectsHeader.createEl("span", {
					cls: "task-group-icon",
				});
				setIcon(projectsIcon, "tag");
				projectsHeader.createEl("span", { text: ` Tags` });
				const projectsList = projectsEl.createEl("div", {
					cls: "project-tags",
				});
		...
		const allPendingForCheck = data.tasks.pending || [];
		if (
			allPendingForCheck.length === 0 &&
	...
	private renderGoals(container: HTMLElement, data: DashboardData) {
		...
		data.goals.active.forEach((goal) => {
			...
			const titleEl = goalEl.createEl("h4", {
				cls: "goal-title clickable",
			});
			titleEl.setText(goal.title);

			// Log progress value for debugging
			console.debug(
				`VaultMind: Goal "${goal.title}" has progress: ${goal.progress}%`
			);
	...
	private renderTimeTracking(container: HTMLElement, data: DashboardData) {
		...
		if (data.time.currentSession) {
			...
			const durationEl = sessionEl.createEl("p", {
			...
			const updateDuration = () => {
				...
				const startTime = new Date(
					data.time.currentSession!.startTime
				).getTime();
		...
		if (data.time.recentEntries.length > 0) {
			...
			const listContainer = recentEl.createDiv({
				cls: "recent-entries-list",
			});
			...
			if (data.time.recentEntries.length > 10) {
				...
				moreEl.setText(
					`... and ${data.time.recentEntries.length - 10} more`
				);
				moreEl.addClass("vaultmind-text-muted");
				moreEl.addClass("vaultmind-italic");
			}
		}
	}
	 ...
	 */
	private applyTagFilter(tasks: VaultMindTask[]): VaultMindTask[] {
	 ...
	 */
	private applyPriorityFilter(tasks: VaultMindTask[]): VaultMindTask[] {
	 ...
	 */
	private sortTasksByPriority(tasks: VaultMindTask[]): VaultMindTask[] {
	 ...
	 */
	private getShowMoreText(count: number, total: number): string {
		if (count < total) {
			const remaining = total - count;
			const showCount = Math.min(10, remaining);
			return `Show ${showCount} more (${remaining} remaining)`;
		}
		return "Show less";
	}
}
...
class TaskSelectionModal extends Modal {
	private tasks: VaultMindTask[];
	private onSelect: (task: VaultMindTask | null) => void;

	constructor(
		app: App,
		tasks: VaultMindTask[],
		onSelect: (task: VaultMindTask | null) => void
	) {
		super(app);
		this.tasks = tasks;
	...
	onOpen() {
		...
		// Task list container
		const taskListContainer = contentEl.createDiv({
			cls: "task-selection-list",
		});
		...
		const renderTasks = (filter: string = "") => {
			...
			filteredTasks.slice(0, 20).forEach((task) => {
				const taskItem = taskListContainer.createDiv({
					cls: "task-selection-item",
				});
				...
				// Add metadata (tags, due date)
				const metadata = [];
				if (task.tags && task.tags.length > 0)
					metadata.push(task.tags[0]);
				if (task.dueDate)
					metadata.push(
