    ...
    display(): void {
        ...
        new Setting(containerEl)
            ...
            .addDropdown(dropdown => dropdown
                .addOption('local', 'Local (FLAN-T5)')
                .addOption('ollama', 'Ollama (External)')
        ...
        if (this.plugin.settings.aiProvider === 'local') {
            new Setting(containerEl)
                .setName('Local Model')
                .setDesc('Choose the local model to use (Only transformers.js supported models)')
                .addDropdown(dropdown => dropdown
                    .addOption('flan-t5-small', 'FLAN-T5 Small (250MB, Recommended)')
                    .addOption('flan-t5-base', 'FLAN-T5 Base (990MB, Better quality)')
                    .addOption('all-minilm-l6', 'MiniLM L6 (90MB, Embeddings only)')
                    .setValue(this.plugin.settings.localModelName)
                    .onChange(async (value) => {
                        this.plugin.settings.localModelName = value;
                        await this.plugin.saveSettings();
                        new Notice('Model changed. Restart required for changes to take effect.');
                    }));

            new Setting(containerEl)
                .setName('Download Model')
                .setDesc('Download the selected model for offline use')
                .addButton(button => button
                    .setButtonText('Download')
                    .onClick(async () => {
                        try {
                            // Create AI provider if it doesn't exist
                            if (!this.plugin.ai) {
                                console.log('VaultMind: Creating AI provider for download...');
                                const { LocalAI } = await import('../ai/LocalAI');
                                this.plugin.ai = new LocalAI(this.plugin.settings);
                                console.log('VaultMind: AI provider created');
                            }
                            
                            if (this.plugin.ai && 'downloadModel' in this.plugin.ai) {
                                new Notice('Starting model download...');
                                await (this.plugin.ai as any).downloadModel(
                                    this.plugin.settings.localModelName, 
                                    this.plugin.app
                                );
                            } else {
                                new Notice('AI provider does not support downloads');
                                console.error('VaultMind: AI provider missing downloadModel method');
                            }
                        } catch (error) {
        ...
        if (this.plugin.settings.enableWebSearch) {
            ...
            if (this.plugin.settings.webSearchProvider === 'brave' || this.plugin.settings.webSearchProvider === 'custom') {
                new Setting(containerEl)
                    .setName('Search API Key')
                    .setDesc(`API key for ${this.plugin.settings.webSearchProvider} search`)
                    .addText(text => text
        ...
        new Setting(containerEl)
            ...
            .setDesc('Allow optional cloud AI features (data will be sent externally)')
            .addToggle(toggle => toggle
        ...
        new Setting(containerEl)
            ...
            .addButton(button => button
                ...
                .setWarning()
                .onClick(async () => {
                    if (confirm('Are you sure? This will delete all VaultMind data!')) {
                        const storage = this.plugin.vaultIndexer.storage;
                        await storage.clear();
