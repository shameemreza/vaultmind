...
export class AdvancedChatView extends ItemView {
    ...
    async onOpen() {
        ...
        attachBtn.addEventListener('click', () => this.openAdvancedAttachModal());
        ...
        this.inputEl = inputContainer.createEl('textarea', {
            ...
            attr: { 
                placeholder: 'Ask about your vault, tasks, goals, or anything else...',
                rows: '2'
            }
        });
...
• **Tasks**: List, filter, analyze (${this.plugin.taskEngine?.getTasks().length || 0} tasks indexed)
• **Goals**: Track progress, suggest next steps (${this.plugin.goalEngine?.getGoals().length || 0} goals found)
...
Use the paperclip button to attach specific files/folders, or I'll search your entire vault automatically!`,
    ...
    private async getEnhancedAIResponse(message: string): Promise<{
        ...
        if (!this.plugin.aiProvider) {
            ...
            if (!this.plugin.aiProvider) {
                return {
                    content: '⚠️ Please configure an AI provider (OpenAI, Claude, or Ollama) in settings.'
                };
            }
        }

        // Build comprehensive context (with fallback if indexer not ready)
        const context = await this.buildComprehensiveContext(message).catch(err => {
            console.warn('VaultMind: Error building context, using fallback', err);
            return `User query: ${message}\nAttached notes: ${this.attachedNotes.map(n => n.basename).join(', ')}`;
        });
        ...
        switch (intent.type) {
            ...
            default:
                // General question
                response = await this.plugin.aiProvider.answerQuestion(message, context);
        }
        ...
        return {
            ...
            context: {
                ...
                tasks: this.includeTasks ? this.plugin.taskEngine?.getTasks().length : 0,
                goals: this.includeGoals ? this.plugin.goalEngine?.getGoals().length : 0,
                searchResults
            }
        };
    }
    ...
    private detectIntent(message: string): {type: string, query?: string} {
        ...
        if (lower.includes('task') || lower.includes('todo') || lower.includes('overdue') || lower.includes('due')) {
            return { type: 'task_query' };
        }
        
        if (lower.includes('goal') || lower.includes('objective') || lower.includes('milestone') || lower.includes('progress')) {
        ...
        for (const pattern of notePatterns) {
            ...
            if (match) {
                return { type: 'note_search', query: match[1]?.trim() || message };
            }
        }
        
        // Check for generic search/find/show/read terms
        if (lower.includes('find') || lower.includes('search') || lower.includes('show') || 
            lower.includes('list') || lower.includes('read') || lower.includes('open') ||
    ...
    private async handleTaskQuery(message: string, context: string): Promise<string> {
        ...
        if (lower.includes('overdue')) {
            relevantTasks = tasks.filter(t => t.dueDate && new Date(t.dueDate) < new Date() && !t.completed);
            title = 'Overdue Tasks';
        } else if (lower.includes('today')) {
            const today = new Date().toDateString();
            relevantTasks = tasks.filter(t => t.dueDate && new Date(t.dueDate).toDateString() === today);
            title = "Today's Tasks";
        ...
        if (relevantTasks.length === 0) {
            ...
            response += `Found **${relevantTasks.length}** ${title.toLowerCase()}:\n\n`;
    ...
    private async handleGoalQuery(message: string, context: string): Promise<string> {
        ...
        if (goals.length === 0) {
            ...
            sorted.forEach(goal => {
                ...
                const status = goal.status === 'completed' ? '[COMPLETED]' : goal.status === 'active' ? '[ACTIVE]' : '[PAUSED]';
                ...
                if (goal.targetDate) {
                    ...
                    response += `Target: ${new Date(goal.targetDate).toLocaleDateString()} (${days} days)\n`;
                }
    ...
    private async handleNoteSearch(
        ...
        if (targetNote) {
            ...
            const cleanTarget = targetNote.toLowerCase()
            ...
            if (normalizedMatch) {
                const content = await this.plugin.app.vault.read(normalizedMatch);
            ...
            const partialMatches = allFiles.filter(f => {
                ...
                return fileName.includes(normalizedTarget) || normalizedTarget.includes(fileName);
            });
            
            if (partialMatches.length === 1) {
                const content = await this.plugin.app.vault.read(partialMatches[0]);
        ...
        if (searchResults.length === 0) {
            ...
            const fallbackResults = allFiles.filter(f => {
                const fileName = f.basename.toLowerCase();
                return fileName.includes(queryLower) || queryLower.includes(fileName);
            }).slice(0, 5);
            ...
            if (searchResults.length > 10) {
                response += `... and ${searchResults.length - 10} more results\n`;
            }
        }
    ...
    private async handleSummarization(message: string, context: string): Promise<string> {
        if (this.attachedNotes.length > 0) {
            // Summarize attached notes
            const contents = await Promise.all(
                this.attachedNotes.slice(0, 5).map(f => this.plugin.app.vault.read(f))
            );
    ...
    private async searchNotes(query: string): Promise<Array<{path: string, title: string}>> {
        // Ensure vaultIndexer is available
        if (!this.plugin.vaultIndexer) {
            console.warn('VaultMind: VaultIndexer not initialized, using fallback');
        ...
        try {
            ...
            return searchResults.map((note: any) => ({
                path: note.path,
            ...
            return scored.map((item: any) => ({
                path: item.note.path || item.note.filePath || '',
                title: item.note.title || item.note.path?.split('/').pop()?.replace('.md', '') || 'Untitled'
            }));
        }
    }
    ...
    private async buildComprehensiveContext(query: string): Promise<string> {
        ...
        if (!this.plugin.vaultIndexer) {
            ...
            return `User query: ${query}\n\nAttached files: ${this.attachedNotes.map(n => n.basename).join(', ')}`;
        }
    ...
    private groupTasks(tasks: any[]): Record<string, any[]> {
    ...
    private async generateVaultSummary(): Promise<string> {
        ...
        summary += `• **Tasks**: ${tasks.length} (${tasks.filter(t => !t.completed).length} pending)\n`;
        summary += `• **Goals**: ${goals.length} (${goals.filter(g => g.status === 'active').length} active)\n`;
        summary += `• **Total Words**: ${notes.reduce((sum, n) => sum + n.wordCount, 0).toLocaleString()}\n\n`;
        ...
        recent.forEach(note => {
            summary += `• [[${note.filePath}|${note.title}]] - ${note.lastModified.toLocaleDateString()}\n`;
        });
    ...
    private openAdvancedAttachModal() {
        const modal = new AdvancedAttachModal(
            ...
            (files, folders) => {
                ...
                this.updateContextIndicator();
                new Notice(`Attached ${files.length} files and ${folders.length} folders`);
            }
        );
        modal.open();
    }
    ...
    private async addMessage(message: ChatMessage, skipPush: boolean = false) {
        ...
        if (message.context) {
            ...
            if (message.context.searchResults?.length) {
                items.push(`${message.context.searchResults.length} search results`);
            }
    ...
    private loadSessions() {
        ...
        if (saved) {
            try {
                ...
                this.sessions = new Map(Object.entries(data).map(([id, session]: [string, any]) => {
                    return [id, {
                        ...session,
...
class AdvancedAttachModal extends Modal {
    ...
    constructor(app: App, plugin: VaultMindPlugin, onSelect: (files: TFile[], folders: TFolder[]) => void) {
        super(app);
        this.plugin = plugin;
    ...
    onOpen() {
        ...
        const filesTab = tabContainer.createEl('button', { text: 'Files', cls: 'tab-button active' });
        const foldersTab = tabContainer.createEl('button', { text: 'Folders', cls: 'tab-button' });
        const recentTab = tabContainer.createEl('button', { text: 'Recent', cls: 'tab-button' });
        
        // Content container
        const contentContainer = contentEl.createEl('div', { cls: 'content-container' });
        
        // Files view
        const filesView = contentContainer.createEl('div', { cls: 'tab-content active' });
        const fileList = filesView.createEl('div', { cls: 'file-list' });
        
        // Folders view
        const foldersView = contentContainer.createEl('div', { cls: 'tab-content' });
        const folderList = foldersView.createEl('div', { cls: 'folder-list' });
        
        // Recent view
        const recentView = contentContainer.createEl('div', { cls: 'tab-content' });
        const recentList = recentView.createEl('div', { cls: 'file-list' });
        
        // Tab switching
        const switchTab = (activeTab: HTMLButtonElement, activeView: HTMLElement) => {
            [filesTab, foldersTab, recentTab].forEach(tab => tab.removeClass('active'));
            [filesView, foldersView, recentView].forEach(view => view.removeClass('active'));
            activeTab.addClass('active');
            activeView.addClass('active');
        };
        
        filesTab.addEventListener('click', () => switchTab(filesTab, filesView));
        foldersTab.addEventListener('click', () => switchTab(foldersTab, foldersView));
        recentTab.addEventListener('click', () => switchTab(recentTab, recentView));
        ...
        const renderFiles = (files: TFile[], container: HTMLElement) => {
            ...
            files.slice(0, 50).forEach(file => {
                const fileEl = container.createEl('div', { cls: 'selectable-item' });
                const checkbox = fileEl.createEl('input', { 
                    type: 'checkbox'
                }) as HTMLInputElement;
                checkbox.checked = this.selectedFiles.has(file);
                const nameEl = fileEl.createEl('span', { text: file.basename });
        ...
        const renderFolders = () => {
            ...
            folders.forEach(folder => {
                const folderEl = folderList.createEl('div', { cls: 'selectable-item' });
        ...
        renderFolders();
        renderFiles(allFiles.sort((a, b) => b.stat.mtime - a.stat.mtime).slice(0, 20), recentList);
        ...
        attachBtn.addEventListener('click', () => {
            this.onSelect(Array.from(this.selectedFiles), Array.from(this.selectedFolders));
            this.close();
        });
...
class VaultSearchModal extends Modal {
    ...
    constructor(app: App, plugin: VaultMindPlugin, onSearch: (query: string) => Promise<void>) {
        super(app);
        this.plugin = plugin;
...
class ContextSettingsModal extends Modal {
    ...
    private onSave: (settings: any) => void;

    constructor(app: App, settings: any, onSave: (settings: any) => void) {
        super(app);
    ...
    onOpen() {
        ...
        new Setting(contentEl)
            ...
            .setDesc('Give AI access to entire vault (may be slow for large vaults)')
