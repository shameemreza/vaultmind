	...
	private async generateDailyReport(): Promise<void> {
		...
		if (summary) {
			...
			if (existingFile instanceof TFile) {
				await this.plugin.app.vault.modify(existingFile, summary);
			} else {
...
`;
	...
	private checkTaskReminders(): void {
		...
		const now = new Date();
		const reminderTime =
			this.plugin.settings.reminderAdvanceTime * 60 * 1000;
			...
			if (task.dueDate) {
				...
				if (timeToDue > 0 && timeToDue <= reminderTime) {
					this.plugin.notificationService.notify({
						type: "reminder",
						title: "Task Due Soon",
						message: `"${task.content}" is due in ${Math.floor(
							timeToDue / 60000
						)} minutes`,
						persistent: true,
						source: "task",
